...
async def translate_sentence_async(
    ...
    """
    Async version of translate_sentence.
    ...
    try:
        logger.info(f"Async translating from {src_lang_name} to {tgt_lang_name}")

        # Get translation using async LiteLLM
        translated_text = (await config.translate_async(prompt)).strip()
