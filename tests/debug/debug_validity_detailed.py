...
def debug_validity_detailed(file_path):
    ...
    for spine_id, _ in reader.book.spine:
        ...
        if item_name in toc_names:
            if current_group:
                ...
                if merged is None:
                    print(f"INVALID: None (content could not be merged from group {current_group})")
                    continue
                ...
                if not is_valid:
                    ...
                    if reader._looks_like_toc(soup, text.lower()):
                        print(f"  - Reason: Looks like TOC")
                    if reader._looks_like_copyright(text.lower()):
                        print(f"  - Reason: Looks like Copyright")
                else:
                    print(f"VALID: {title}")
            current_group = [item]
        else:
    ...
    if current_group:
        merged = reader._merge_items(current_group)
        if merged is None:
            print(f"INVALID: None (content could not be merged from final group {current_group})")
        else:
            title = reader.get_chapter_title(merged)
